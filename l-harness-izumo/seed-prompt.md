...
2. scenario.json の内容をそのスキーマに変換して登録する
   ...
   - タグ9種
   - ステップ配信 Day1〜Day7（Day2のみタグでレベル分岐。配信時刻は全て20:00、タイムゾーンは Asia/Tokyo）
3. メッセージ本文は messages/ の各ファイルの「---」より下の本文部分だけを使う（見出しと注意書きは登録しない）
4. `{{report_url}}` `{{counseling_url}}` `{{course_url}}` は scenario.json の variables の値で置換する。まだ example.com のままなので、Day3・Day5・Day7 は下書き（無効）状態で登録し、URL確定後に有効化できるようにしておく
...
- 決済URLが決まったら `scenario.json` の `variables` を書き換え → 再シード →
